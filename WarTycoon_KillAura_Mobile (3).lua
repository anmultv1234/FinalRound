-- War Tycoon Kill Aura Mobile v7
-- CRITICAL FIX: Capture FireRocket:InvokeServer return value for RocketHit ID
-- RocketHit now uses server-assigned rocket ID instead of self-generated counter
-- FireRocketClient called in ALL modes for server-side rocket tracking
-- Added debug prints to verify remote calls are working
...
local function fireRpg(tool, t)
    ...
    -- CRITICAL: FireRocket:InvokeServer returns a server-assigned rocket ID
    -- This ID MUST be used for RocketHit, NOT a self-generated counter
    local rocketId = nil
    pcall(function()
        rocketId = r.fire:InvokeServer(dir, tool, tool, t.pos)
    end)

    if S.debug and rocketId then
        print("[KA] FireRocket returned ID:", rocketId, type(rocketId))
    end

    -- FireRocketClient for server-side rocket tracking (called in ALL modes)
    ...
    -- RocketHit for instant damage (explode mode)
    if S.explode and r.hit and t.head then
        -- Build the rocket hit ID from the server-returned value
        -- Server expects: PlayerName .. "Rocket" .. serverAssignedNumber
        local hitId
        if rocketId then
            if type(rocketId) == "number" then
                hitId = LP.Name .. "Rocket" .. tostring(rocketId)
            else
                hitId = tostring(rocketId)
            end
        else
            -- Fallback: use old format if InvokeServer returned nil
            rocketNum = rocketNum + 1
            hitId = LP.Name .. "Rocket" .. tostring(rocketNum)
            if S.debug then
                print("[KA] WARNING: InvokeServer returned nil, using fallback ID:", hitId)
            end
        end

        if S.debug then
            print("[KA] RocketHit ID:", hitId)
        end
...
pcall(function()
    local old = parent:FindFirstChild("WT_KA_v7")
    if old then
        old:Destroy()
    end
end)
...
info.Text = "v7 | Range 8000 | Q toggle"
...
refresh()
print("[KA] v7 loaded parent=" .. tostring(sg.Parent))
