-- War Tycoon Kill Aura Mobile v5
-- Fixed: RocketHit ID format, nilWeapon mesh, ACS Equip, getnilinstances polyfill
...
-- getnilinstances polyfill (unsafe = nil on most mobile executors)
...
local function findNilWeapon(name)
    ...
    -- fallback: search BulletFireSystem descendants
...
local function fireRpg(tool, t)
    ...
    -- InvokeServer returns rocket ID for Explode path
    local ok, rn = pcall(function()
        return r.fire:InvokeServer(dir, tool, tool, t.pos)
    end)
    local rid = tostring(rn or rocketNum)
    ...
    if S.explode and r.hit and t.head then
        pcall(function()
            r.hit:FireServer(
                t.pos,      -- targetPos
                dir,        -- lookVector/direction
                tool,       -- weapon
                tool,       -- weapon
                t.head,     -- headPart (Hit part)
                t.head,     -- headPart again
                LP.Name .. "RK" .. rid
            )
        end)
    end
...
local function fireGun(tool, t)
    ...
    pcall(function()
        b.fire:FireServer(
            { dir },                  -- direction in table
            tool,                     -- weapon Tool
            nilWep or tool,           -- nil-parented mesh or fallback
            t.pos,                    -- target position
            false                     -- ADS off
        )
    end)
    ...
    if b.hit and t.head then
        pcall(function()
            b.hit:FireServer(
                ...
                tool,
                nilWep or tool,
                t.head,               -- Head part -> direct TakeDamage
                t.head,
...
pcall(function()
    local old = parent:FindFirstChild("WT_KA_v5")
    if old then
        old:Destroy()
    end
end)
...
refresh()
print("[KA] v5 loaded parent=" .. tostring(sg.Parent))
