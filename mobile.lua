...
if not getgenv().ScriptState then
    getgenv().ScriptState = {
        ...
        HitboxEnabled = false,
        HitboxSize = 2,
        antiRpgTriggerActive = false,
        antiRpgTriggerReady = false,
        isFlyActive = false,
...
RageBox:AddToggle("AntiRpgSpam_Enabled", {
    ...
    Callback = function(value)
        if not value then
            ScriptState.isLockedOn = false
            ScriptState.targetPlayer = nil
            ScriptState.antiRpgTriggerActive = false
            ScriptState.antiRpgTriggerReady = false
            ...
            local EspGlobal = getgenv().ExunysDeveloperESP
            if EspGlobal then
                if EspGlobal.Loaded then
                    EspGlobal.Settings.MaxDistance = 9e9
                    EspGlobal.Settings.Enabled = true
                elseif EspGlobal.Load then
                    pcall(function()
                        EspGlobal:Load()
                    end)
                    task.wait(0.5)
            ...
            task.delay(1, function()
                if Toggles.AntiRpgSpam_Enabled.Value then
                    ScriptState.antiRpgTriggerReady = true
                end
            end)
        end
    end
...
RunService.Heartbeat:Connect(function()
    ...
    if targetRoot and targetHum and targetHum.Health > 0 then
        ...
        if not targetChar:FindFirstChildOfClass("Highlight") then
        ...
        if dist > 8000 then
            myRoot.CFrame = myRoot.CFrame + dir * 50
        ...
        if dist >= 6000 and dist <= 8000 and ScriptState.antiRpgTriggerReady then
            if not ScriptState.antiRpgTriggerActive then
                ScriptState.antiRpgTriggerActive = true
            end
...
task.spawn(function()
    ...
    Main:AddToggle("HitboxToggle", {
        ...
        Default = SilentAimSettings.HitboxEnabled,
        Tooltip = "Expands player head size",
        Callback = function(value)
            SilentAimSettings.HitboxEnabled = value
    ...
    Main:AddSlider("HitboxSize", {
        Text = "Hitbox Size",
        Default = SilentAimSettings.HitboxSize,
        Min = 1,
        Max = 20,
        Rounding = 0,
        Tooltip = "Adjust hitbox size",
        Callback = function(value)
            SilentAimSettings.HitboxSize = value
...
ACSEngineBox:AddToggle("Spoof_Enabled", {
    Text = "Enable Fire Mods (Auto)",
    Default = false,
    Callback = function(value)
        ScriptState.Spoof_Enabled = value
        if value then
            local gunsFolder = game:GetService("ReplicatedStorage"):FindFirstChild("Configurations")
            if gunsFolder and gunsFolder:FindFirstChild("ACS_Guns") then
                for _, gunConfig in pairs(gunsFolder.ACS_Guns:GetChildren()) do
                    if gunConfig:FindFirstChild("Settings") then
                        local settings = require(gunConfig.Settings)
                        settings.Mode = ScriptState.Spoof_Mode or "Auto"
                        settings.FireModes = settings.FireModes or {}
                        settings.FireModes.Auto = true
                        settings.FireRate = ScriptState.Spoof_FireRate or 1000
...
AACBox:AddToggle("AAC_Enabled", {
    Text = "Enable AAC",
    Default = false,
    Callback = function(value)
        if AACActivateButton then
            AACActivateButton.Visible = value
        end
    end
})
...
AACBox:AddLabel("PC Combo Keybind"):AddKeyPicker("AAC_ActivateKey", {
    Default = "None",
...
local AACActivateButton = Instance.new("TextButton")
AACActivateButton.Parent = ScreenGui
AACActivateButton.BackgroundColor3 = Color3.fromRGB(25, 25, 25)
AACActivateButton.Size = UDim2.new(0, 80, 0, 30)
AACActivateButton.Position = UDim2.new(1, -100, 0.5, 105)
AACActivateButton.Text = "ACTIVATE"
AACActivateButton.TextColor3 = Color3.fromRGB(255, 255, 255)
AACActivateButton.Font = Enum.Font.Code
AACActivateButton.TextSize = 14
AACActivateButton.BorderSizePixel = 0
AACActivateButton.Active = true
AACActivateButton.Visible = false

local AACUIStroke = Instance.new("UIStroke")
AACUIStroke.Thickness = 1.5
AACUIStroke.Color = Color3.fromRGB(255, 165, 0)
AACUIStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
AACUIStroke.Parent = AACActivateButton

AACActivateButton.MouseButton1Down:Connect(function()
    ScriptState.AAC_State = "Initializing"
    ScriptState.stopFrameCount = 0
    ScriptState.lastMoveDirection = Vector3.zero
    AACUIStroke.Color = Color3.fromRGB(0, 255, 0)
end)

AACActivateButton.MouseButton1Up:Connect(function()
    ScriptState.AAC_State = "Idle"
    AACUIStroke.Color = Color3.fromRGB(255, 165, 0)
end)

local aacDragging, aacDragInput, aacDragStart, aacStartPos
local function aacUpdate(input)
    local delta = input.Position - aacDragStart
    AACActivateButton.Position = UDim2.new(
        aacStartPos.X.Scale,
        aacStartPos.X.Offset + delta.X,
        aacStartPos.Y.Scale,
        aacStartPos.Y.Offset + delta.Y
    )
end

AACActivateButton.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
        aacDragging = true
        aacDragStart = input.Position
        aacStartPos = AACActivateButton.Position
...
AACActivateButton.InputChanged:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then
        aacDragInput = input
    end
end)
...
game:GetService("UserInputService").InputChanged:Connect(function(input)
    if input == aacDragInput and aacDragging then
        aacUpdate(input)
    end
end)
...
local function executeAACCombo()
    ...
    if not hum then
        ScriptState.AAC_State = "Idle"
        if AACUIStroke then AACUIStroke.Color = Color3.fromRGB(255, 165, 0) end
        return
    end

    local targetPart, targetPlayer = getClosestPlayer({
        ignoredPlayers = Options.PlayerDropdown and Options.PlayerDropdown.Value
    })
    ...
    ScriptState.aacTarget = targetPlayer
    ...
    if Options.AAC_Mode.Value == "PatZoom" then
        ...
        if ScriptState.aacTarget then
            ScriptState.targetPlayer = ScriptState.aacTarget
            ScriptState.isLockedOn = true
            SilentAimSettings.Enabled = true
        ...
        if not oldLock then ScriptState.isLockedOn = false end
...
RunService.Heartbeat:Connect(function()
    ...
    if root and hum then
        ...
        if ScriptState.lastRootPosition then
            if (currentPos - ScriptState.lastRootPosition).Magnitude < 0.5 then
